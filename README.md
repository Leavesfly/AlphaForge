...
- AI 对话式交互，自然语言驱动分析任务
- **next_steps 链式引导**：结果尾部附可选下一步建议（模拟盘跟踪/止损告警/逐只三灯等），SSE 独立事件下发，点击即跳转对应页面

### 🚦 买点三灯决策台
- **价/势/时三灯**（估值硬伤否决 + 趋势分权重模型 + 过热/回撤/量价背离）→ 七态行动结论（趋势买点/纯趋势仓/等回踩/左侧观察/回避/减风险/无法评分）
- ATR 交易计划：入场区间、止损（2×ATR）、2R/3R 目标、风险预算法建议仓位
- **用户风险画像**：保守/平衡/激进三档仓位乘数（×0.5/×1.0/×1.5），对话可自然语言设置
- **持仓联动**：已登记持仓自动带出成本与浮盈，结论切换"减仓纪律"视角；扫描结果标注已持有
- 数据缺失诚实降级：估值不可用→价灯灰灯并明示"结论仅基于势/时"；K 线不足→无法评分不猜测
...
- 信号准确率追踪与策略权重动态调优
- Verifier 层信号校验
- **数据源主动体检**：逐源真实拉取探测（耗时/末根K线日期/错误分类），区分环境问题（限流/无Key/网络）与代码问题；串行执行 + TTL 缓存，不加剧上游限流
...
| 模块 | 路径 | 说明 |
|------|------|------|
| 系统 | `/health`, `/dashboard`, `/health/datasources` | 健康检查、仪表盘、数据源主动体检（`?force=true` 绕过缓存） |
| 分析 | `/analysis/**` | 触发分析、查看报告 |
| 信号 | `/signals/**` | 决策信号 CRUD、反馈 |
| 选股 | `/screening/**` | AlphaSift 智能选股（候选标注 held） |
| 决策 | `/decision/score?code=&cost=` | 买点三灯评估（含 next_steps 建议） |
| 画像 | `/profile` | 用户风险画像 GET/PUT（仓位乘数） |
...
| 告警 | `/alerts/**` | 规则、触发、通知 |
| 对话 | `/chat/**` | AI 对话会话管理（SSE 含 tool/chunk/next_steps/done/error 事件） |
| 认证 | `/auth/**` | 登录、Token 刷新 |
