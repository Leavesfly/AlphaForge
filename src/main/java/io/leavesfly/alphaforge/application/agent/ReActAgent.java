...
public class ReActAgent {
    ...
    private static final String SYSTEM_PROMPT_TEMPLATE = """
            ...
            当需要获取实时数据或操作 L4 自主闭环时，你可以通过 Function Calling 调用工具。
            系统已将工具定义注册为原生 Function Calling，你只需根据需要选择调用即可。

            可用工具：
            %s

            ## L4 自主控制（autonomy_control）
            当用户提到：打开/关闭自主、自动执行信号、绑定模拟账户、查自主状态、查审计、熔断、恢复交易时，
            必须调用 autonomy_control，不要只口头回答。先执行工具再向用户确认结果。
